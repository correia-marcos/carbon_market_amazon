...
redd_aud_valid <- filter_type_variation(final_buffer, type = "AUD", tolerance = 0.1,
                                        additional = FALSE)
redd_apd_valid <- filter_type_variation(final_buffer, type = "APD", tolerance = 0.1,
                                        additional = TRUE)

verra_apd <- rbind(verra_1147, verra_1382, verra_832, verra_2551) %>% 
  duplicate




df_aud <- scpi::scdataMulti(redd_aud_valid, id.var = "id_registry",
                            outcome.var = "Total_natural_formation",
                            treatment.var = "treatment",
                            time.var = "year", constant = TRUE)

df_apd <- scpi::scdataMulti(redd_apd_valid, id.var = "id_registry",
                            outcome.var = "Total_natural_formation",
                            treatment.var = "treatment",
                            time.var = "year", constant = TRUE)
...
# Plotting
plots_aud <- scplotMulti(result_aud, e.out = TRUE, ncols = 6)
plots_apd <- scplotMulti(result_apd, e.out = TRUE, ncols = 6)
...
df_aud_att <- scdataMulti(redd_apd_valid, id.var = "id_registry",
                          outcome.var = "Total_natural_formation",
                          treatment.var = "treatment",
                          time.var = "year",
                          constant = TRUE,
...
### Load data
data <- verra_832_conservation

donors <- data %>%
  filter(!id_registry == "832")
...
period.pre  <- seq(from = first(data$year), to = 2007, by = 1)    # Pre-treatment period
period.post <- (2008:2022)                            # Post-treatment period
unit.tr     <- "832"                         # Treated unit (in terms of id.var)
unit.co     <- setdiff(unique(data$id_registry), unit.tr) # Donors pool
outcome.var <- "Total_natural_formation"              # Outcome variable
cov.adj     <- NULL                                   # Covariates for adjustment
features    <- NULL                                   # No features other than outcome
constant    <- FALSE                                  # No constant term
report.missing <- TRUE                               # To check where missing values are
...
scplot(result = result, fig.path = ".",
       fig.name = "verra_1147", fig.format = "png", plot.range = (2000:2020),
       label.xy = list(x.lab = "Year", y.lab = "Percentage of Natural Formation"),
       x.ticks = NULL, e.out = T, event.label = list(lab = "REDD", height = 9))
